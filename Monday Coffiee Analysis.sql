...
SELECT  city_name,
	ROUND((population * 0.25), 2) as coffee_consumers_in_millions
FROM city
...
SELECT * 
FROM (SELECT ci.city_name,p.product_name,
		COUNT(s.sale_id) as total_orders,
...
AS(SELECT 
		...
		SUM(s.total) as total_revenue,
		COUNT(DISTINCT s.customer_id) as total_cx,
		ROUND(SUM(s.total) * 1.0 / COUNT(DISTINCT s.customer_id), 2) AS avg_sale_pr_cx  
...
WITH monthly_sales AS (
    ...
    GROUP BY 
        ...
        MONTH(sale_date), 
        YEAR(sale_date)
    -- Note: ORDER BY is not allowed inside a standard CTE view, 
    -- but LAG handles the ordering for us below anyway.
),
growth_ratio AS (
    SELECT
        city_name,
...
WITH city_table AS (
  ...
  city_rent AS (
    ...
    ROUND((population * 0.25) / 1000000.0, 3) AS estimated_coffee_consumer_in_millions
    FROM city
)
SELECT 
      cr.city_name,  ct.total_revenue,cr.estimated_rent AS total_rent, ct.total_cx, cr.estimated_coffee_consumer_in_millions, ct.avg_sale_pr_cx,
    ROUND(cr.estimated_rent * 1.0 / ct.total_cx,2) AS avg_rent_per_cx
FROM city_rent AS cr
